...
Feature: TC_002_Logout and session invalidation behavior
  ...
  @happy_path
  Scenario: TS_001_User logs out successfully
    When the user clicks the Logout button in the header
    Then the user should be redirected to the pre-login page


  @security @no-cache
  Scenario: TS_002_Back navigation after logout does not restore session
    When the user clicks the Logout button in the header
